...
func run_tls_proxy(proxy_conf ProxyTemplate) {
	...
	proxy_serv.HandleFunc("/", func(w http.ResponseWriter, req *http.Request) {
		...
		log.Printf("%+v", req)
		req_full, err := httputil.DumpRequest(req, true)
		if err == nil {
			log.Print(string(req_full))
		}

		tls_conf := &tls.Config{InsecureSkipVerify: true}
		proxy_client := &http.Client{
			Transport: &http.Transport{
				TLSClientConfig: tls_conf,
				TLSNextProto:    make(map[string]func(authority string, c *tls.Conn) http.RoundTripper),
				DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
					// hack to bypass DNS and fix SNI
					dialer := net.Dialer{}
					if addr == req.Host+":443" || addr == "www."+req.Host+":443" {
						addr = proxy_conf.Endpoint + ":443"
					}
		...
		log.Printf("%+v", res)
        // WARNING: your logs will be VERY large...
		//res_full, err := httputil.DumpResponse(res, true)
		//if err == nil {
		//    log.Print(string(res_full))
		//}
		io.Copy(w, res.Body)
	})
